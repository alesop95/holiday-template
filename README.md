...
Seconda operazione: scrivi `trips/<nuovo-nome>/trip.config.js` da zero, sul modello di `trips/cilento-2026/trip.config.js`. Imposta un `TRIP_ID` nuovo e univoco (es. `"tokyo-2026"`) e lascia `FIREBASE_CONFIG` e `TRIP_PLANNER_URL` identici a quelli degli altri viaggi: sono rispettivamente lo stesso progetto Firebase condiviso (`viaggio-new`) e lo stesso backend Render condiviso, quindi non serve crearne uno nuovo né richiedere nuove credenziali. Modifica poi `TRIP_META`, `MAP_LOCATIONS` e `TRIP_DATA.days / .restaurants / .checklist` con i contenuti del nuovo viaggio; la struttura degli oggetti rimane identica, cambiano solo i valori. `ROUTING_PROFILE` è un export obbligatorio, non opzionale (un import ES nominato mancante rompe il caricamento dell'intero modulo nel browser): va impostato a `"driving"` o `"foot"` secondo il mezzo prevalente del nuovo viaggio, non copiato per inerzia da un viaggio con un mezzo diverso, perché cambia la stima di tempo/distanza mostrata dal pulsante "Ottimizza percorso" nella scheda "Itinerario". Stesso vincolo di obbligatorietà per `CURRENCY_CODE` (codice ISO 4217, es. `"EUR"`, `"USD"`, `"GBP"`) e `CURRENCY_SYMBOL` (es. `"€"`, `"$"`, `"£"`): impostarli sulla valuta reale del nuovo viaggio, non su quella di un viaggio precedente copiato per comodità, perché vengono passati ai tre servizi backend con prezzi (`flight-search`, `stay-search`, `trip-planner`) come parametro di ricerca reale, non solo usati per un'etichetta a schermo.
...
Il primo comando può fallire se `holiday-template-<nuovo-nome>` è già in uso da un altro progetto Firebase nel mondo (i site-id sono globali): in quel caso basta un suffisso diverso, senza che cambi nient'altro nella procedura. Il secondo comando scrive la mappatura nel file `.firebaserc` locale della cartella (gitignored): per questo la chiave `"target"` dentro `firebase.json` (valore uguale a `<nuovo-nome>`, cioè il nome della cartella) va scritta a mano nel file copiato al primo passo, mentre `.firebaserc` la riceve in automatico dal comando CLI.

Quarta operazione, **allargare la restrizione referrer della apiKey su Google Cloud Console** (progetto `viaggio-new` > API e servizi > Credenziali > "Browser key (auto created by Firebase)" > Restrizioni relative ai siti web): aggiungere `https://holiday-template-<nuovo-nome>.web.app/*` e `https://holiday-template-<nuovo-nome>.firebaseapp.com/*` alla lista, poi salvare. **Un carattere jolly non basta**: un pattern come `https://holiday-template-*.web.app/*` viene rifiutato dalla Console ("Dominio sito web non valido") perché il carattere jolly di Google sostituisce un'intera etichetta di sottodominio, non una porzione di un'etichetta — verificato in sessione, non un'ipotesi. Questo significa due righe manuali in più per ogni nuovo viaggio, non una regola unica valida per sempre; se questo passo viene saltato, l'app resta bloccata sulla schermata di caricamento con un errore Firebase (sezione 7.1, gestito dal blocco `try/catch` di `init()`) anche con `FIREBASE_CONFIG` corretto, perché Firebase rifiuta la richiesta dal referrer non ancora autorizzato — un sintomo diverso, non confonderlo con un `apiKey` non sostituita.

Al primo accesso all'URL generato, `seedIfNeeded()` (sezione 7.2) rileverà che `trips/{TRIP_ID}/content/days` non esiste ancora per questo nuovo `TRIP_ID` e popolerà Firestore con i nuovi dati, senza toccare quelli di nessun altro viaggio.
...
**Non esiste più** una collection Firestore `trips/{TRIP_ID}/content` da editare da Console per aggiornamenti rapidi: `days`, `restaurants` e `checklist` (insieme a `hotelChange`, `accommodation`, `costEstimate`, `tickets`, `savingTips`, `programSummary`) si leggono sempre da `TRIP_DATA` in `trip.config.js` a ogni caricamento della pagina, mai da Firestore. Prima erano seminati una sola volta al primo accesso e restavano congelati lì: comodo per editare da Console senza redeploy, ma fonte reale di confusione durante lo sviluppo attivo di un viaggio, perché ogni modifica successiva al file restava invisibile finché qualcuno non cancellava a mano i documenti già seminati. Ogni modifica al contenuto del viaggio richiede quindi un `firebase deploy` (sezione 11), non un'alternativa da Console. Solo `trips/{TRIP_ID}/state/*` (checklist spuntata, note, alloggi salvati in Pianifica, meta modificato dall'hero, costi) resta su Firestore, seminato una sola volta e poi genuinamente posseduto dall'utente: quello, giustamente, non si tocca da un redeploy.
